...
class TextureAtlas:
    ...
    def __init__(self,assets):
        self.assets=assets;self.rects={};self.models={};self.faces={}
    ...
    def face(self,block,direction):
        ...
        choice=variants.get('',next(iter(variants.values()),{}))
        if isinstance(choice,list):choice=choice[0] if choice else {}
        model=self.model(choice.get('model',f'{namespace}:block/{ident}'))
        ...
        if ref is None:
            candidates=(direction,'top' if direction=='up' else 'bottom' if direction=='down' else 'side','end' if direction in ('up','down') else 'side','all','texture')
        ...
        path=next((p for p in paths if p in self.rects),None)
        ...
        # Approximate biome tint for greyscale foliage/water; other textures stay uncoloured.
        tinted=('leaves' in ident or ident in ('water','grass','short_grass','fern','vine') or ident=='grass_block' and direction=='up')
