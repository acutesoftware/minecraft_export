...
Java 1.12 numeric IDs and packed metadata now share one decoder across near meshes, distant terrain and maps. This preserves dyed blocks, wood species and stone variants; optional `Add` nibbles preserve extended IDs. The numeric registry is based on [PrismarineJS minecraft-data's 1.12 block registry](https://github.com/PrismarineJS/minecraft-data/blob/master/data/pc/1.12/blocks.json). The local 1.12 client assets use bare model names, older blockstate names (including `silver` for light gray), and `textures/blocks/`; these are resolved alongside modern assets.

Fences use narrow posts and neighbour-connected rails, including connections across chunk boundaries. Gates use approximate open/closed geometry. Neither hides adjacent cube faces. Entity-rendered blocks such as chests, beds, signs and shulker boxes still use approximate geometry/colours; their entity textures and block-entity colour data are not loaded. Other complex shapes and orientations remain approximations.

Cache version `mesh-10` invalidates older meshes for the legacy decoding, texture resolution and fence fixes; map tiles use `tiles-4-legacy-variants`. Restart the viewer and load from the source world to regenerate. Previously saved 3D exports retain their original mesh data and must be regenerated to show these changes. Local database, textures embedded in it, caches and diagnostic renders remain under Git-ignored paths.
